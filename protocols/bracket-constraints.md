...
1. **What is the deck's target bracket?** (Check `overview.md`)
2. **Is the card on the Game Changers list?**
    - If yes: does adding it keep the deck within its Game Changer budget?
3. **Does the card enable or accelerate a two-card infinite combo?**
    - If yes: is that combo early-game? → Flag for bracket violation
...
6. **Does the card's _intent_ match the target bracket?**
